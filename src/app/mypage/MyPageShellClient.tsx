...
import { myPageSectionRoutes, type MyPageSection } from "./myPageTypes";
import { useMypage } from "./MypageProvider";

const selectedSectionClass = "bg-brand-highlight/70 text-text-strong";

type MyPageShellClientProps = {
  children: ReactNode;
};

export function MyPageShellClient({
  children,
}: MyPageShellClientProps) {
  const pathname = usePathname();
  const { canShowAccountSettings, t } = useMypage();
...
const resolveActiveSection = (pathname: string | null): MyPageSection => {
