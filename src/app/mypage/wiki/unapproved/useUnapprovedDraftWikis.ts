...
export const useUnapprovedDraftWikis = ({
  ...
  const approveMutation = useMutation<unknown, Error, MyPageWikiListItem>({
  ...
  const rejectMutation = useMutation<unknown, Error, { reason: string; wiki: MyPageWikiListItem }>({
  ...
  const approveDraftWiki = (wiki: MyPageWikiListItem) => {
    approveMutation.mutate(wiki);
  };

  const rejectDraftWiki = (wiki: MyPageWikiListItem, reason: string) => {
