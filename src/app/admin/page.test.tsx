...
import type {
  AdminAccountSettingsTab,
  AdminRouteContext,
  AdminSection,
  AdminSettingsTab,
  AdminWikiTab,
} from "./adminTypes";
import type { AdminDraftWikiActionTab } from "./useAdminDraftWikis";
...
const createAdapter = (
  overrides: Partial<AdminPrincipalAdapter> = {},
): AdminPrincipalAdapter => ({
...
const createDraftImageAdapter = (
  overrides: Partial<AdminDraftImageAdapter> = {},
): AdminDraftImageAdapter => ({
...
const createDraftWikiAdapter = (
  overrides: Partial<AdminDraftWikiAdapter> = {},
): AdminDraftWikiAdapter => ({
...
const emptyDraftWikiLists: Record<AdminDraftWikiActionTab, typeof emptyDraftWikiListState> = {
...
function AdminClient({
  ...
  initialAccountSettingsTab?: AdminAccountSettingsTab;
  initialDraftWikis?: AdminRouteContext["initialDraftWikis"];
  initialIdentity: AdminRouteContext["initialIdentity"];
  initialPrincipalState?: AdminRouteContext["initialPrincipalState"];
  initialSection?: AdminSection;
  initialSettingsTab?: AdminSettingsTab;
  ...
  return (
    <div onClickCapture={handleRouteClick}>
      <AdminProvider initialContext={context}>
        <AdminShellClient>{activeContent}</AdminShellClient>
      </AdminProvider>
    </div>
  );
}
...
const getInitialAdminPathname = ({
  ...
  if (initialSection === "accountSettings") {
    ...
    if (initialAccountSettingsTab === "principalGroupManagement") {
      return "/admin/account/principal-groups";
    }
  ...
  if (initialWikiTab === "submittedWikis") {
    return "/admin/wiki/submitted";
  }

  if (initialWikiTab === "approvedWikis") {
    return "/admin/wiki/approved";
  }

  if (initialWikiTab === "unapprovedWikis") {
    return "/admin/wiki/unapproved";
  }

  if (initialWikiTab === "untranslatedWikis") {
    return "/admin/wiki/untranslated";
  }

  if (initialWikiTab === "draftImages") {
    return "/admin/wiki/draft-images";
  }

  if (initialWikiTab === "imageDeletionRequests") {
    return "/admin/wiki/image-deletion-requests";
  }

  return "/admin/wiki/editing";
};
...
describe("admin page clients", () => {
  ...
  it("renders the sidebar with Wiki selected by default", async () => {
    ...
    expect(screen.getByRole("complementary", { name: "管理画面メニュー" })).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "管理画面メニューを閉じる" }),
  ...
  it("collapses and expands the sidebar with a persistent toggle", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "管理画面メニューを閉じる" }));
    expect(
      screen.getByRole("button", { name: "管理画面メニューを開く" }),
    ).toHaveAttribute("aria-expanded", "false");

    fireEvent.click(screen.getByRole("button", { name: "管理画面メニューを開く" }));
    expect(
      screen.getByRole("button", { name: "管理画面メニューを閉じる" }),
    ).toHaveAttribute("aria-expanded", "true");
  });
