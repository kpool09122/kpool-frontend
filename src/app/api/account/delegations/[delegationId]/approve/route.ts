...
import { getAccountApiBaseUrl, parseAccountDelegationSummary } from "@/gateways/account/accountApi";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const apiResponse = await fetch(`${baseUrl}/delegations/${encodeURIComponent(delegationId)}/approve`, { method: "POST", headers: getForwardHeaders(request, false), cache: "no-store" });
