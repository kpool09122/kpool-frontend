...
test("admin shows the Wiki collaborator activation path", async ({ page }) => {
  ...
  await expect(page.getByRole("complementary", { name: "管理画面メニュー" })).toBeVisible();
  await expect(
    page.getByRole("button", { name: "管理画面メニューを閉じる" }),
  ).toHaveAttribute("aria-expanded", "true");
  await page.getByRole("button", { name: "管理画面メニューを閉じる" }).click();
  await expect(
    page.getByRole("button", { name: "管理画面メニューを開く" }),
  ).toHaveAttribute("aria-expanded", "false");
  await page.getByRole("button", { name: "管理画面メニューを開く" }).click();
  await expect(
...
test("admin shows under review draft images for an available Wiki principal", async ({ page }) => {
  await useJapaneseLocale(page);
  ...
  await expect(page).toHaveURL(/\/admin\/wiki\/draft-images$/);
...
test("admin keeps wiki tabs interactive after browser back", async ({ page }) => {
  await useJapaneseLocale(page);
  await useWikiPrincipal(page, "image-review");
...
test("admin shows unapproved draft wikis only for reviewer principals", async ({ page }) => {
  await useJapaneseLocale(page);
  ...
  await expect(page).toHaveURL(/\/admin\/wiki\/unapproved$/);
...
test("admin publishes approved draft wikis only for publisher principals", async ({ page }) => {
  await useJapaneseLocale(page);
  ...
  await expect(page).toHaveURL(/\/admin\/wiki\/approved$/);
...
test("admin translates untranslated wikis for publisher principals", async ({ page }) => {
  await useJapaneseLocale(page);
  await useWikiPrincipal(page, "wiki-publish");
  ...
  await expect(page).toHaveURL(/\/admin\/wiki\/untranslated$/);
...
test("admin hides draft image review for principals without image policies", async ({ page }) => {
  await useJapaneseLocale(page);
...
test("admin lets account policy users edit account information", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/admin\/account\/profile$/);
